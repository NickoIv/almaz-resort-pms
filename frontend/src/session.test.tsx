 ...
 * меню. Ни то, ни другое не было ошибкой самой страницы: у токена кончился
 * срок, вкладку на ночь не закрывают, утренний опрос получил 401, `api`
...
describe('§43 истёкший токен уводит на вход, а не в красную полосу', () => {
  ...
  it('и говорит, почему спрашивает PIN снова', async () => {
    ...
    // Иначе повторный PIN выглядит так, будто приложение забыло прошлый вход
    // просто так, — и следующий вопрос будет «оно сломалось?».
    await screen.findByText(/Вход действует сутки/)
  })
...
describe('§43 неверный PIN — это не истёкшая сессия', () => {
  it('говорит про PIN и молчит про срок входа', async () => {
    mockApi({
      'POST /api/auth/login': () =>
    ...
    // «Вход действует сутки» человеку, который промахнулся по цифре, — это
    // подсказка не про то, и она уводит от настоящей причины.
    await waitFor(() => expect(screen.queryByText(/Вход действует сутки/)).toBeNull())
  })
})
