 ...
 *
 * Токен живёт сутки (`TOKEN_TTL_SECONDS`). Вкладку на стойке не закрывают, так
 * что рано или поздно она доживает до конца срока: опрос отвечает 401,
 * `setToken(null)` убирает токен — и на этом всё останавливалось. `user` при
 * этом оставался в памяти React, поэтому шапка по-прежнему называла
 * администратора по имени, меню работало, а каждая страница показывала красное
 * «Missing bearer token»: заголовок без токена — то, чем становится любой
 * следующий запрос после того, как токен убрали.
 *
