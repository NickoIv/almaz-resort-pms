...
CREATE TABLE IF NOT EXISTS push_subscriptions (
  ...
  /**
   * The push service's URL for this browser install. Unique across the whole
   * table on purpose: an endpoint identifies one browser, so when a second
   * member of staff signs in on a shared device and subscribes, the row must
   * move to them rather than leave the previous person receiving that device's
   * notifications.
   */
  ...
  last_ok_at    TEXT,
  /**
   * Consecutive send failures. A push service answers 404/410 once a
   * subscription is dead, and those rows are deleted outright; this counts the
   * softer failures so a permanently broken endpoint stops being retried.
   */
...
--
-- Alert ids are stable and encode what makes an event new (see routes/alerts),
-- so "have I sent this?" is a primary-key lookup. Without it the five-minute
-- sweep would re-send every outstanding alert twelve times an hour.
